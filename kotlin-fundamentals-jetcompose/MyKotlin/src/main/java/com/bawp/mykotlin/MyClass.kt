...
fun main() {
  /**
   * var - mutable val - immutable Best practice - In essence, use val over var
   *
   * val name: String = "James" - explicitly initializing a variable
   *
   * Another way to initialize variables val name: String name = "James" Basic Types in Kotlin:
   * (Whole Numbers)
   * - Int
   * - Byte
   * - Short Floating Points Types: (Decimal Numbers):
   *     - Double
   *     - Float Note: for precision, use Double. val e = 2.7182818284 val eFloat = 2.7182818284f
   *       val amount = 0 when (amount) { in 1..100 -> print("This amount is between 1 and 100") !in
   *       10..90 -> print("This amount is outside the range") 999 -> print("Really Close") 1000 ->
   *       print("Rich but not there...") 1100 -> print("You've made it!") else -> { print("$amount
   *       is just not going to work") } }
   *
   * // calculate(first = 12, // second = 199, // message = "Hey", // multipleOf = 2) // calculate(
   * message = "Hello there", multipleOf = 3) val catAge = calculateCatAge(age = 2) if
   * (checkAge(catAge)) print("This cat is old ($catAge)") else print("This cat is young ($catAge)")
   *
   * //Collections - Introduction val myListOfNames = listOf("James", "Paul", "Rafael", "Gina") val
   * myMutableList = mutableListOf(12, 34, 45, 123)
   *
   * println("Number of elements ${myMutableList.size}") println("Second element
   * ${myMutableList[1]}") println("Index of element \"45\" ${myMutableList.indexOf(45)}")
   *
   * for (item in myMutableList) { println(item) }
   *
...
fun createGenerics() {
  ...
  val aroni = Person(name = "Ruti", lastName = "Machava", age = 54)
  val ruti = Person(name = "Ruti", lastName = "Machava", age = 54)
  val listOfPeople = listOf(person, aroni, ruti)
  val listOfNumbers = listOf(23, 45, 45)
  val finder = Finder(list = listOfPeople)
  finder.findItem(element = aroni) { println("Found $it") }
}
...
fun kCollections() {
  ...
  val found =
      myListOfNames.filter {
        it.startsWith("r", ignoreCase = true) && it.endsWith('L', ignoreCase = true)
      }
  print(found)
}

val name: (String) -> Unit = { print("Hello, my name is $it") }
...
fun calculate(first: Int = 10, second: Int = 1000, message: String, multipleOf: Int) {
